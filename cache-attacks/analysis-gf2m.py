...
log = []
aux_time = ""
for line in open(dir+"sect163r1.log"):
    if "WARNING" in line:
        continue
    if "cycles" in line:
        continue
...
for idx in range(len(traces_begin)):
        ...
        idx2 = 0
        while idx2 < len(trace_b)-1:
                # Look for the first hit on the start monitor
                if (trace_b[idx2] == 1) and (start == 0):
                        if (idx2 < 35) or (idx2 > 50):
                                v = -1
                                break
                        start = idx2+1
                        idx2 = start+1
                ...
                if (trace_e[idx2] == 1) and (start > 0):
                        if (idx2 - start) > 15 and ((trace_b[idx2] == 1) or (trace_b[idx2 + 1] == 1)):
                                v = 1
                                guess_one += 1
                        else:
                                v = 0
